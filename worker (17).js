...
async function readBoard(env) {
  ...
  if (!record.vasSchedule || typeof record.vasSchedule !== "object" || Array.isArray(record.vasSchedule)) record.vasSchedule = {};
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/appdata" && request.method === "GET") {
        ...
        return json({
          data: {
            ...
            vasSchedule: record.vasSchedule || {},
          },
          role: auth.role,
          email: auth.email,
          group: auth.group,
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        const scheduleFields = ["midweekSchedule", "studySchedule", "weekendSchedule", "vasSchedule"];
