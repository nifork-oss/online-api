...
async function readBoard(env) {
  const res = await fetchJsonBinWithRetry(`https://api.jsonbin.io/v3/b/${BIN_ID}/latest?t=${Date.now()}`, {
    headers: { "X-Master-Key": env.JSONBIN_KEY, "Cache-Control": "no-cache" },
  ...
  // Архив — прошедшие недели переносятся сюда вручную, чтобы не
  // загромождать основной список, но оставались доступны при необходимости.
  if (!record.midweekArchive || typeof record.midweekArchive !== "object" || Array.isArray(record.midweekArchive)) record.midweekArchive = {};
  if (!record.studyArchive || typeof record.studyArchive !== "object" || Array.isArray(record.studyArchive)) record.studyArchive = {};
  if (!record.weekendArchive || typeof record.weekendArchive !== "object" || Array.isArray(record.weekendArchive)) record.weekendArchive = {};
  if (!record.vpsArchive || typeof record.vpsArchive !== "object" || Array.isArray(record.vpsArchive)) record.vpsArchive = {};
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // Смена собственного пароля — любой вошедший может сменить ТОЛЬКО
      // свой пароль, обязательно подтвердив текущий.
      if (path === "/change-password" && request.method === "POST") {
        const body = await request.json();
        const currentPassword = String(body.currentPassword || "");
        const newPassword = String(body.newPassword || "").trim();
        if (!newPassword || newPassword.length < 4) {
          return json({ error: "Новый пароль слишком короткий (минимум 4 символа)" }, 400);
        }
        const record = await readBoard(env);
        const user = record.users.find((u) => u.email === auth.email);
        if (!user) return json({ error: "Пользователь не найден" }, 404);
        if (!(await verifyPassword(currentPassword, user.pass))) {
          return json({ error: "Текущий пароль указан неверно" }, 403);
        }
      ...
      if (path === "/appdata" && request.method === "GET") {
        const record = await readBoard(env);
        const currentUserRecordForAccess = record.users.find((u) => u.email === auth.email);
        return json({
          data: {
            ...
            midweekArchive: record.midweekArchive || {},
            studyArchive: record.studyArchive || {},
            weekendArchive: record.weekendArchive || {},
            vpsArchive: record.vpsArchive || {},
            reorderAccessRequests: isSuperadmin ? record.reorderAccessRequests : record.reorderAccessRequests.filter((r) => r.email === auth.email),
          },
          role: auth.role,
          email: auth.email,
          group: auth.group,
          canReorderOwnGroup: hasReorderAccess(currentUserRecordForAccess),
          canReorderOwnGroupExpiresAt: currentUserRecordForAccess ? (currentUserRecordForAccess.canReorderOwnGroupExpiresAt || null) : null,
        });
      }
      ...
      if (path === "/appdata" && request.method === "PUT") {
            ...
            if (valid) {
              for (const r of newList) {
                ...
                if (String(prev.group) === myGroup) {
                  // Запись из его группы — группу и букву менять нельзя, позицию можно.
                  if (String(r.group) !== myGroup || String(r.letter || "") !== String(prev.letter || "")) { valid = false; break; }
                } else {
                  // Запись из чужой группы — должна остаться нетронутой полностью.
                  if (String(r.group) !== String(prev.group) || String(r.position) !== String(prev.position) || String(r.letter || "") !== String(prev.letter || "")) { valid = false; break; }
                }
              }
            }
        ...
        const scheduleFields = ["midweekSchedule", "studySchedule", "weekendSchedule", "vpsSchedule", "midweekTrash", "studyTrash", "weekendTrash", "vpsTrash", "midweekDraft", "studyDraft", "weekendDraft", "vpsDraft", "midweekArchive", "studyArchive", "weekendArchive", "vpsArchive"];
        ...
        await writeBoard(env, record);
        const currentUserRecordForAccessFinal = record.users.find((u) => u.email === auth.email);
        return json({
          data: {
            users: isSuperadmin ? stripPasswords(record.users) : [],
            groupRoster: record.groupRoster || [],
          ...
          scheduleLastSeen: (currentUserRecordForAccessFinal || {}).scheduleLastSeen || {},
          canReorderOwnGroup: hasReorderAccess(currentUserRecordForAccessFinal),
          canReorderOwnGroupExpiresAt: currentUserRecordForAccessFinal ? (currentUserRecordForAccessFinal.canReorderOwnGroupExpiresAt || null) : null,
        });
      }
