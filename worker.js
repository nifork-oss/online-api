  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/login" && request.method === "POST") {
        ...
        return json({ token, email: user.email, role: user.role, group: user.group, approved: isApproved, canReorderOwnGroup: hasReorderAccess(user), scheduleLastSeen: user.scheduleLastSeen || {}, _debug: { rawCanReorderOwnGroup: user.canReorderOwnGroup, rawExpiresAt: user.canReorderOwnGroupExpiresAt, now: Date.now() } });
      }
