  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/login" && request.method === "POST") {
        ...
        return json({ token, email: user.email, role: user.role, group: user.group, approved: isApproved, canReorderOwnGroup: !!user.canReorderOwnGroup, scheduleLastSeen: user.scheduleLastSeen || {} });
      }
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        if (Array.isArray(incoming.groupRoster)) {
          if (isAdmin) {
            ...
            if (!currentUserRecord || !currentUserRecord.canReorderOwnGroup) {
